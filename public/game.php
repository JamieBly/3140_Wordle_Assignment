<?php 
    $myFile = fopen("docs\\5LetterWords.txt", "r") or die("Unable To Open Word Dictionary");
    $myDictionary = fread($myFile, filesize("docs\\5LetterWords.txt"));
    $myDictionary = explode(",", $myDictionary);
    $myWord = $myDictionary[rand(0, count($myDictionary) -1)];
    setcookie("myWord", $myWord);
...
<html lang="en-us">
    <head>
        ...
        <link rel="stylesheet" href="app/wordle.css">
        
        <title><?php echo $myWord?></title>
    </head>
    <body>
        <!-- Header that contains the Logo, which also acts as a home button -->
        <div class="header">
            <a href="index.php">
                <img src="docs/wordlebyjamie_logo.png" id="wordlelogo"></img>
            </a>
        </div>
